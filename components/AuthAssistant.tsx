...
export const AuthAssistant: React.FC = () => {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="btn btn-warning rounded-circle shadow-lg d-flex align-items-center justify-content-center position-fixed floating-chat-btn"
  ...
  return (
    <div
      className="position-fixed bottom-0 end-0 w-100 h-100 sm-w-96 sm-h-600 bg-white rounded-top-4 sm-rounded-4 shadow-lg d-flex flex-column z-50 animate-slide-up"
      style={{ maxWidth: "384px", maxHeight: "600px" }}
    >
      ...
      <div className="flex-grow-1 p-3 overflow-y-auto bg-light">
        <div className="d-flex flex-column gap-3">
          {messages.map((msg, index) => (
            <div
              key={index}
              className={`d-flex align-items-end gap-2 ${msg.sender === "user" ? "justify-content-end" : "justify-content-start"}`}
            >
              {msg.sender === "ai" && (
                <div
                  className="rounded-circle bg-warning text-dark d-flex align-items-center justify-content-center flex-shrink-0"
                  style={{ width: "32px", height: "32px" }}
                >
                  <i className="fas fa-question-circle small"></i>
                </div>
              )}
              <div
                className={`position-relative group p-3 rounded-3 fs-6 ${
                  msg.sender === "user"
                    ? "bg-primary text-white rounded-bottom-end-0"
                    : "bg-body-secondary text-body rounded-bottom-start-0"
                } ${msg.sender === "system" ? "bg-danger-subtle text-danger-emphasis text-center w-100" : ""}`}
                style={{ maxWidth: "80%" }}
              >
                {msg.text}
                {msg.sender === "ai" && (
                  <button
                    ...
                    className="position-absolute btn btn-sm btn-light rounded-circle d-flex align-items-center justify-content-center opacity-0 group-hover-opacity-100"
                    style={{
                      bottom: "-10px",
                      right: "-10px",
        ...
        <div className="d-flex align-items-center gap-2">
          {recognition && (
            <button
              ...
              className={`btn rounded-circle d-flex align-items-center justify-content-center flex-shrink-0 ${isListening ? "btn-danger" : "btn-light"}`}
          ...
          <button
            ...
            className="btn btn-warning rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
            style={{ width: "50px", height: "50px" }}
          >
