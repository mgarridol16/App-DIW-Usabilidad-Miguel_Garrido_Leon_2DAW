...
import { EducationalInfographics } from "./EducationalInfographics";
...
export const LessonView: React.FC<LessonViewProps> = ({
  ...
  const handleCheckAnswer = async () => {
    ...
    const isCorrect = await onSubmission(
      lesson.id,
      currentStepIndex,
      selectedAnswer,
    );
  ...
  const renderInstructionalStep = (step: Step) => (
    <>
      <div className="d-flex justify-content-between align-items-start">
        <h3 className="h4 fw-semibold text-body-secondary flex-grow-1">
          {step.title}
        </h3>
        {currentSkin !== "alternative" && (
          <button
            onClick={() => speak(step.content || "")}
            className="btn btn-light rounded-circle d-flex align-items-center justify-content-center ms-3 lesson-tts-button"
            style={{ width: "50px", height: "50px" }}
            aria-label="Leer instruccion en voz alta"
          >
      ...
      {step.video ? (
        <video
          ...
          src={step.video}
          controls
          className="w-100 rounded-3 shadow-sm aspect-video bg-dark"
        />
      ) : (
        <div className="w-100 rounded-3 shadow-sm aspect-video bg-light border d-flex align-items-center justify-content-center p-4">
          ...
          ) : step.image && !step.image.startsWith("//") ? (
            <EducationalInfographics type={step.image} />
          ) : (
            <p className="text-muted fst-italic text-center">
              [{step.image?.replace("// Placeholder: ", "")}]
            </p>
          )}
        </div>
      )}
      ...
      <div className="d-flex flex-column gap-3">
        {step.options?.map((option: QuizOption) => {
          ...
          if (answerStatus !== "unanswered" && isSelected) {
            btnClass =
              answerStatus === "correct" ? "btn-success" : "btn-danger";
          } else if (isSelected) {
          ...
          return (
            <button
              ...
              className={`btn ${btnClass} lesson-answer-option text-start p-3 fs-5 rounded-3 d-flex align-items-center gap-3`}
              disabled={answerStatus !== "unanswered" || isSubmitting}
            >
              <div
                className={`border rounded-circle d-flex flex-shrink-0 ${isSelected ? "bg-primary border-primary" : "border-secondary"}`}
    ...
    return (
      <div className="text-center p-5 bg-white rounded-4 shadow-lg">
        ...
        <p className="text-muted mt-2">
          Ha ocurrido un problema al cargar este paso. Por favor, vuelva al
          inicio.
        </p>
        <button onClick={onGoBack} className="btn btn-primary mt-4 px-4 py-2">
          Volver al Menú
        </button>
      </div>
    );
    ...
    <div className="lesson-view animate-fade-in">
      ...
      <div className="bg-white p-4 p-md-5 rounded-4 shadow-lg border">
        <header className="mb-4 pb-4 border-bottom">
          <div className="d-flex justify-content-between align-items-start">
            <h2 className="h1 fw-bold text-primary-emphasis flex-grow-1 pe-4">
              {lesson.title}
            </h2>
            <div className="d-flex align-items-center gap-2">
              {Array.from({ length: MAX_LIVES }).map((_, i) => (
                <i
                  key={i}
                  className={`fas fa-heart fs-2 ${i < lives ? "text-danger" : "text-light"}`}
                ></i>
              ))}
            </div>
          </div>
          <div className="progress mt-4" style={{ height: "10px" }}>
            <div
              ...
              aria-valuenow={
                (currentStepIndex / (lesson.steps.length - 1)) * 100
              }
              aria-valuemin={0}
              aria-valuemax={100}
            ></div>
        ...
        >
          {currentStep.type === "instructional" &&
            renderInstructionalStep(currentStep)}
          {currentStep.type === "multipleChoice" &&
            renderMultipleChoiceStep(currentStep)}
          {currentStep.type === "simulatorTask" && (
            <PracticeSimulator
      ...
      <div
        className={`mt-2 rounded-bottom-3 transition-all duration-300
      ...
      >
        {answerStatus === "correct" && (
          <p className="fw-bold fs-5 mb-0">
            {currentStep.feedbackCorrect || "¡Buen trabajo!"}
          </p>
        )}
        {answerStatus === "incorrect" && (
          <p className="fw-bold fs-5 mb-0">
            {currentStep.feedbackIncorrect || "¡Ups! Inténtalo de nuevo."}
          </p>
        )}
      </div>

      <div className="mt-4 pt-4 d-flex justify-content-between align-items-center gap-4">
        <button onClick={onGoBack} className="btn btn-danger fw-semibold">
          Salir
        </button>
        <button
          onClick={() => {
          ...
          disabled={
            isSubmitting ||
            (currentStep.type === "multipleChoice" && !selectedAnswer) ||
            (currentStep.type === "simulatorTask" && answerStatus !== "correct")
          }
